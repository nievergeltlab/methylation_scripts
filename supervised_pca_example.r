#Test for differential methylation using supervised PCA
...
superpc_train_dat <- list(x=as.matrix(dat_training),y = covariates_ordered_training$PTSDbroad,censoring_status=NULL,features=row.names(as.matrix(dat_training)))
superpc_test_dat <- list(x=as.matrix(dat_testing),y = covariates_ordered_testing$PTSDbroad,censoring_status=NULL,features=row.names(as.matrix(dat_testing)))

#train the pca using training data
trained <- superpc.train(superpc_train_dat,type="regression")

#cross validate the training data...
cved <- superpc.cv(trained,superpc_train_dat)

#plot the cv results
superpc.plotcv(cved)

#precit testing data using training data
predictions <- superpc.predict(trained, superpc_train_dat, superpc_test_dat, threshold=1.85, n.components=3,prediction.type="discrete")

superpc.predictionplot(trained, superpc_train_dat, superpc_test_dat, threshold=1.85,n.components=3)


##################### alternative model where cell types are decorrelated from the data....
...
#decorrelate data to competing predictors
...
#make train and test data sets
superpc_train_dat_decor <- list(x=t(dat_training_decor$res),y = covariates_ordered_training$PTSDbroad,censoring_status=NULL,features=row.names(as.matrix(dat_training)))
superpc_test_dat_decor <- list(x=t(dat_testing_decor$res),y = covariates_ordered_testing$PTSDbroad,censoring_status=NULL,features=row.names(as.matrix(dat_testing)))


#train the pca using training data
trained_decor <- superpc.train(superpc_train_dat_decor,type="regression")

#cross validate the training data...
cved_decor <- superpc.cv(trained_decor,superpc_train_dat_decor,n.components=3)

#plot the cv results
superpc.plotcv(cved_decor)

#Plot likelhiood ratio test statistics from output of superpc.predict
lrtest_outs <- superpc.lrtest.curv(object=trained_decor, data=superpc_train_dat_decor, newdata=superpc_test_dat_decor, n.components = 1, threshold = NULL, n.threshold = 30)
superpc.plot.lrtest(lrtest_outs)

#precit testing data using training data
predictions <- superpc.predict(trained_decor, superpc_train_dat, superpc_test_dat, threshold=1.8, n.components=3,prediction.type="continuous")

#plot predictions using difffent thresholds and n pcs
superpc.predictionplot(train.obj=trained_decor, data=superpc_train_dat_decor, data.test=superpc_test_dat_decor, threshold=.75,n.components=3)

#use linear model prediction
modelfit <- superpc.fit.to.outcome(fit=trained_decor, data.test=superpc_test_dat_decor,score=predictions$v.pred, iter.max=10)

#add in more competing predictors...
cpreds_testing2 <- list (pred1=covariates_ordered_testing$DRRI_composite_i, pred2=covariates_ordered_testing$plate_combat)
modelfit2 <- superpc.fit.to.outcome(fit=trained_decor, data.test=superpc_test_dat_decor,competing.predictors=cpreds_testing2,score=predictions$v.pred, iter.max=5)
